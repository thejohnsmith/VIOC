...
var StorePageDetailController = StorePageDetailController || (function ($) {
	...
	var controller = {
		...
		attachEventListeners: function () {
			...
			$("input[name='storeNeighboringType']").change(function () {
				controller.onChangePreviewInput()
			});
			$(targets.join(",")).change(function () {
				controller.onChangePreviewInput()
			});
			$(targets.join(",")).keyup(function () {
				controller.onChangePreviewInput()
			});
		...
		getLandmarkPlaceholder: function (key) {
			...
			case "input[name='storeNeighboringType']:checked":
				...
				break;
			case "#storeNeighboring":
				placeholder = "[NEIGHBORING BUSINESS]";
				break;
			case "#storeCrossStreet":
				placeholder = "[CROSS STREET]";
				break;
			case "#storeLandmark":
				placeholder = "[POPULAR LANDMARK]";
				break;
			case "#storeDistDirection":
