...
$(document).ready(function () {
	...
	$('#ctl00_FooterContent2_connect1_btnSignUp').on('click', function (e) { validateConnectForm(e); });
});
	...
	switch(_experiment){
		...
		switch(chosenVariation){
			...
			if(Cookies.get('och') == undefined){
				Cookies.set('och', 'seen', { expires: 60 });
				var initialPopup = $('\
...
function loadDesktop() {
	...
	$('#footer .button').click(function(e){
		...
		setTimeout(function(){eval(myAction);}, 1000);
	});
...
function resetMapInterface() {
	...
	showBigMap();
	showLocalImage({onComplete:function(){
...
function showCitiesList(state) {
	...
	$("#location_column").animate({top: '-710px'}, 400);
	state.animate({'top':-1*(page)*parseInt($('#cities').height())}, 400);
}
                    ...
                    switch (temp.length) {
                        case 2:
                            if (parseInt(temp[1]) + '' == temp[1]) {
                                ...
                                searchFor({ val: temp[1] + ', ' + temp[0] });
                            }
                            break;

                        case 3:
                            searchFor({ val: temp[1] + ', ' + temp[0], goTo: temp[2] });
                            break;
...
function loadReviews(resultNum){
	...
	if(myItem.find('.google_review').length == 0){
		...
		places.nearbySearch(request, function(results, status, pagination) {
			if(results.length > 0){
				var myLoc = results[0];
				places.getDetails({reference:myLoc.reference}, function(place, status){
...
function resultSetNumber(setNumber){
	...
	$('#sidebar_search_results').animate({'margin-top':-1 * (setNumber * parseInt($('#sidebar_search_list_inside').height()))}, 500, function(){
...
function makeMarkers(list) {
    ...
    if(list.length > 0) {
        ...
        $(list).each(function(num, el) {
            ...
            myRight.append('<a href="'+drivingLink({ lat: myLat, lng: myLng }, this.address.address1+' '+this.address.city+', '+this.address.state+' '+this.address.zip)+'" id="driving_directions" target="_blank">GET DRIVING DIRECTIONS</a>');
           ...
           // if (markerIds[this.ID] == undefined) {
                var marker = new google.maps.Marker({ position: (new google.maps.LatLng(this.latitude, this.longitude)), icon: icon, shadow: shadow});
                marker.myId = count;
                var marker2 = new google.maps.Marker({ position: (new google.maps.LatLng(this.latitude, this.longitude)), icon: icon, shadow: shadow });
                var marker3 = new google.maps.Marker({ position: (new google.maps.LatLng(this.latitude, this.longitude)), icon: icon, shadow: shadow });
...
function submitSMS (formID)
{
	...
	if ($id.attr('id') == "smsForm-right")
		$('#enter-mobile').html("<p class=\"enter bold\">Thanks for entering</p><p>You should receive a message shortly with your coupon code.</p>");
	else if ($id.attr('id') == "smsForm-footer")
		$('#phone-form').html("<p class=\"enter bold\">Thanks for entering</p><p>You should receive a message shortly with your coupon code.</p>");
...
var VIOC_17432 = function () {
    ...
    // Accessors/Mutators
    VAPI_getAPI = function () { return api; }
    VAPI_setAPI = function (_api) { api = _api; }
    VAPI_getCookie = function () { return cookie; }
    VAPI_setCookie = function (_cookie) { cookie = _cookie; }
    VAPI_getData = function () { return data; }
                ...
                $.ajax({
                    ...
                    error: function () {
                        ...
                        if (_for == "vehicle") {
                            ...
                            VAPI_call(VAPI_getAPI(), 'stores/trackingID/', VAPI_getCookie(), { 'return_address': 'y', 'return_hours': 'n', 'return_storeOffers': 'n' }, 'store');
