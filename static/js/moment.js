...
(function(global, factory) {
  ...
  function chooseLocale(names) {
    ...
    while (i < names.length) {
      ...
      while (j > 0) {
        ...
        if (next && next.length >= j && compareArrays(split, next, true) >=
          j - 1) {
  ...
  function isFunction(input) {
    return input instanceof Function || Object.prototype.toString.call(
  ...
  function formatMoment(m, format) {
    ...
    formatFunctions[format] = formatFunctions[format] || makeFormatFunction(
      format);
  ...
  function localeMonthsShort(m, format) {
    return isArray(this._monthsShort) ? this._monthsShort[m.month()] :
      this._monthsShort[MONTHS_IN_FORMAT.test(format) ? 'format' :
        'standalone'][m.month()];
  }
  ...
  function localeMonthsParse(monthName, format, strict) {
    ...
    for (i = 0; i < 12; i++) {
      ...
      // test the regex
      if (strict && format === 'MMMM' && this._longMonthsParse[i].test(
          monthName)) {
        return i;
      } else if (strict && format === 'MMM' && this._shortMonthsParse[i].test(
          monthName)) {
        return i;
  ...
  function computeMonthsParse() {
    ...
    this._monthsStrictRegex = new RegExp('^(' + longPieces.join('|') + ')$',
      'i');
    this._monthsShortStrictRegex = new RegExp('^(' + shortPieces.join('|') +
      ')$', 'i');
  }
  ...
  function checkOverflow(m) {
    ...
    if (a && getParsingFlags(m).overflow === -2) {
      overflow =
        ...
        a[HOUR] < 0 || a[HOUR] > 24 || (a[HOUR] === 24 && (a[MINUTE] !== 0 ||
  ...
  addParseToken('YYYY', function(input, array) {
    array[YEAR] = input.length === 2 ? utils_hooks__hooks.parseTwoDigitYear(
      input) : toInt(input);
  });
  ...
  function dayOfYearFromWeekInfo(config) {
    ...
    if (w.GG != null || w.W != null || w.E != null) {
      ...
      // create now).
      weekYear = defaults(w.GG, config._a[YEAR], weekOfYear(
        local__createLocal(), 1, 4).year);
      week = defaults(w.W, 1);
      ...
      weekYear = defaults(w.gg, config._a[YEAR], weekOfYear(
        local__createLocal(), dow, doy).year);
      week = defaults(w.w, 1);
  ...
  function configFromObject(config) {
    ...
    config._a = map([i.year, i.month, i.day || i.date, i.hour, i.minute, i.second,
      i.millisecond
    ], function(obj) {
  ...
  function offset(token, separator) {
    addFormatToken(token, 0, 0, function() {
      ...
      return sign + zeroFill(~~(offset / 60), 2) + separator + zeroFill(~
        ~(offset) % 60, 2);
    });
  }
    ...
    if (input != null) {
      ...
      if (offset !== input) {
        if (!keepLocalTime || this._changeInProgress) {
          add_subtract__addSubtract(this, create__createDuration(input -
            offset, 'm'), 1, false);
  ...
  function create__createDuration(input, key) {
    ...
    if (isDuration(input)) {
      ...
      duration = {};
    } else if (typeof duration === 'object' && ('from' in duration || 'to' in
        duration)) {
  ...
  function moment_calendar__calendar(time, formats) {
    ...
    var output = formats && (isFunction(formats[format]) ? formats[format]() :
      formats[format]);
  ...
  function isSame(input, units) {
    ...
    if (units === 'millisecond') {
      ...
      inputMs = +localInput;
      return +(this.clone().startOf(units)) <= inputMs && inputMs <= +(this
        .clone().endOf(units));
    }
  }
  ...
  function toArray() {
    var m = this;
    return [m.year(), m.month(), m.date(), m.hour(), m.minute(), m.second(),
      m.millisecond()
    ];
  }
  ...
  addWeekParseToken(['gggg', 'ggggg', 'GGGG', 'GGGGG'], function(input,
    week, config, token) {
  ...
  function getSetQuarter(input) {
    return input == null ? Math.ceil((this.month() + 1) / 3) : this.month((
  ...
  function localeWeekdays(m, format) {
    return isArray(this._weekdays) ? this._weekdays[m.day()] :
      this._weekdays[this._weekdays.isFormat.test(format) ? 'format' :
        'standalone'][m.day()];
  }
    ...
    for (i = 0; i < 7; i++) {
      ...
      if (strict && !this._fullWeekdaysParse[i]) {
        this._fullWeekdaysParse[i] = new RegExp('^' + this.weekdays(mom, '')
          .replace('.', '\.?') + '$', 'i');
        this._shortWeekdaysParse[i] = new RegExp('^' + this.weekdaysShort(
          mom, '').replace('.', '\.?') + '$', 'i');
        this._minWeekdaysParse[i] = new RegExp('^' + this.weekdaysMin(mom,
          '').replace('.', '\.?') + '$', 'i');
      }
      if (!this._weekdaysParse[i]) {
        regex = '^' + this.weekdays(mom, '') + '|^' + this.weekdaysShort(
          mom, '') + '|^' + this.weekdaysMin(mom, '');
        this._weekdaysParse[i] = new RegExp(regex.replace('.', ''), 'i');
      }
      // test the regex
      if (strict && format === 'dddd' && this._fullWeekdaysParse[i].test(
          weekdayName)) {
        return i;
      } else if (strict && format === 'ddd' && this._shortWeekdaysParse[i].test(
          weekdayName)) {
        return i;
      } else if (strict && format === 'dd' && this._minWeekdaysParse[i].test(
          weekdayName)) {
        return i;
  ...
  function getSetISODayOfWeek(input) {
    ...
    return input == null ? this.day() || 7 : this.day(this.day() % 7 ?
      input : input - 7);
  }
  ...
  function getSetDayOfYear(input) {
    var dayOfYear = Math.round((this.clone().startOf('day') - this.clone().startOf(
      'year')) / 864e5) + 1;
  ...
  function duration_add_subtract__addSubtract(duration, input, value,
    direction) {
  ...
  function duration_humanize__relativeTime(posNegDuration, withoutSuffix,
    locale) {
