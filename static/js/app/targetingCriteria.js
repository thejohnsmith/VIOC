...
{
	$("#oilType").change(updateRecipientCount);
	$("#chkActive").change(updateRecipientCount);
	$("#chkInactive").change(updateRecipientCount);
	$("#chkNew").change(updateRecipientCount);
	$("#from").blur(updateRecipientCount);
	$("#to").blur(updateRecipientCount);
	$("#fleet").change(updateRecipientCount);
...
var updateRecipientCount = function(save, callback)
{
	...
	{
		if (!isNaN(parseInt(selectedElements[idx])))
				storeIds.push(selectedElements[idx]);
	}
	...
	$.get( "https://adobe-uat-vioc.epsilon.com/jssp/vioc/getRecipientEstimate.jssp", data)
	  .done(function( data ) {
		...
		{	
			...
			if (typeof callback == "function")
				callback(data.recipientCount, data.saveId);
		}
		catch (e)
		{
