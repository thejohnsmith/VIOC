...
var recordLogin = (function ($) {
	...
	var setLandingPageType = function (results) {
		...
		var manageOrdersUrl = marcomUserData.$constants.manageOrdersUrl;
		...
		// If the current user isn't an authorized exception, hide On Demand Marketing.
		if ($.inArray(marcomUserData.$user.externalId, authorizedOrderManagementUsers) == -1) {
			$('.manageOrdersUrl').hide();
		} else {
			$('.menuItem.itemIndex_1').after('<div class="menuItemText manageOrdersUrl"><a href="' + manageOrdersUrl + '" class="itemTitle ellipsis">Manage Orders</a></div>');
		}
		...
		// Hide the Link in the utility nav: Digital Delivery History
		$('.menuItem .menuItemText > a').filter(function () {
			return $(this).text() === 'Digital Delivery History';
		}).hide();
